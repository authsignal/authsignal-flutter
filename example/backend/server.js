    ...
    if (trackResponse.state === 'CHALLENGE_REQUIRED') {
      try {
        ...
        if (deviceAuth) {
          ...
          console.log(`  ✅ Device challenge created: ${deviceChallengeResponse.challengeId}`);
        ...
        console.error('  ⚠️ Device challenge error:', challengeError.message);
...
app.get('/api/user/:userId', async (req, res) => {
  try {
    ...
    console.log(`  ✅ User found with ${userResponse.userAuthenticators?.length || 0} authenticators`);
...
app.listen(PORT, () => {
  ...
  console.log('');
  console.log('💡 For Flutter Android Emulator, use: http://10.0.2.2:${PORT}');
  console.log('');
});
